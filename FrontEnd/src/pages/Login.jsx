...
const Login = () => {
  ...
  return (
    <form className='min-h-[80vh] flex items-center mx-auto'>
      ...
      <div className='flex flex-col gap-3 m-auto items-start p-8 min-w-[340px] sm:min-w-96 border rounded-xl
       text-zinc-600 text-sm shadow-lg'>
        ...
        <p>Please {state === 'Sign Up' ? "sign up" : "log in"} for upcoming event and notification</p>
        {
          state === "Sign Up" && <div className='w-full'>
            <p>Full Name</p>
            <input className='border border-zinc-300 rounded w-full p-2 mt-1' type="text" onChange={(e) => 
              setName(e.target.name)} value={name} required />
          </div>
        }
        ...
        <button className='bg-violet-700 text-white w-full py-2 rounded-md text-base'>
          {state === 'Sign Up' ? "Create Account" : "Login"}</button>
        {
          state === "Sign Up"
            ? <p>Already have an account? <span onClick={() => setState('Login')} 
            className='text-violet-700 underline cursor-pointer'>Login here</span></p>
            : <p>Create an new account? <span onClick={() => setState('Sign Up')} 
            className='text-violet-700 underline cursor-pointer'>Click here</span></p>
        }
      </div>
